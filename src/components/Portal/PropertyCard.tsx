...
const PropertyCard: React.FC<PropertyCardProps> = ({ project, variant = 'list' }) => {
    ...
    return (
        <div className={`group bg-white rounded-[1.5rem] overflow-hidden shadow-[0_2px_10px_rgba(0,0,0,0.04)] hover:shadow-2xl hover:shadow-brand-900/10 transition-all duration-500 border border-slate-100 flex ${isGrid ? 'flex-col h-full' : 'flex-col md:flex-row h-full md:h-[260px]'}`}>
            ...
            {/* Content Section */}
            <div className={`flex-1 flex flex-col justify-between relative ${isGrid ? 'p-5' : 'md:p-6 p-5'}`}>
                ...
                <div>
                    <Link to={`/project/${project.slug}`} className="block group/title">
                        <div className="flex justify-between items-start mb-2">
                            <div className="flex-1 min-w-0 pr-2">
                                <h3 className={`font-bold text-slate-800 leading-snug group-hover/title:text-brand-600 transition-colors mb-1 truncate ${isGrid ? 'text-lg' : 'text-xl md:text-2xl'}`}>
                                    {project.title}
                                </h3>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                        {project.configurations?.slice(0, isGrid ? 2 : 4).map((conf, i) => (
                            <span key={i} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-50 border border-slate-200/60 text-xs font-bold text-slate-600 group-hover:bg-brand-50 group-hover:text-brand-700 group-hover:border-brand-100 transition-colors">
                    ...
                    {!isGrid && (
                        <div className="mt-4 grid grid-cols-2 gap-4 text-sm text-slate-600 border-t border-slate-100 pt-3">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="font-semibold text-slate-700">{project.possessionDate || 'Soon'}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <span className="w-1.5 h-1.5 rounded-full bg-slate-300"></span>
                                <span className="text-slate-400 text-xs uppercase font-bold">Builder:</span>
                                <span className="font-semibold text-slate-700 truncate">{project.builderId || 'Premium Developer'}</span>
                            </div>
                        </div>
                    )}
                </div>
                ...
                {/* Actions Footer */}
                <div className={`mt-5 pt-0 flex gap-3 ${isGrid ? 'flex-col' : 'justify-end items-center'}`}>
                    <button
                        onClick={(e) => {
                            e.preventDefault();
                            window.open(`https://wa.me/917744009295?text=Hi, I am interested in ${project.title}`, '_blank');
                        }}
                        className={`flex-1 px-4 py-2.5 rounded-xl border border-brand-200 text-brand-600 font-bold text-sm hover:bg-brand-50 hover:border-brand-300 transition-all uppercase tracking-wide text-center shadow-sm hover:shadow-md ${isGrid ? 'w-full' : 'max-w-[140px]'}`}
                    >
                        Contact
                    </button>
                    <Link
                        to={`/project/${project.slug}`}
                        className={`flex-1 px-4 py-2.5 rounded-xl bg-brand-600 text-white font-bold text-sm hover:bg-brand-700 transition-all uppercase tracking-wide text-center shadow-lg shadow-brand-200 hover:shadow-brand-300 hover:-translate-y-0.5 ${isGrid ? 'w-full' : 'max-w-[140px]'}`}
                    >
                        Details
                    </Link>
                </div>
