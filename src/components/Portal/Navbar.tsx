...
const PortalNavbar = ({ variant = 'transparent' }: { variant?: 'transparent' | 'colored' }) => {
    ...
    const isSolid = scrolled || variant === 'colored';
    ...
    return (
        <>
            <motion.nav
                className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isSolid ? 'bg-white/95 backdrop-blur-md shadow-glass border-b border-gray-200 py-2' : 'bg-transparent py-4'
                    }`}
            >
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between items-center">
                        ...
                        <Link to="/" className="flex items-center gap-2 group z-50">
                            ...
                            <div>
                                <h1 className={`text-xl font-extrabold tracking-tight transition-colors flex gap-1 ${scrolled ? '' : 'md:text-white'}`}>
                                    <span className={isSolid ? 'text-[#4285F4]' : 'text-white'}>PUNE</span>
                                    <span className={isSolid ? 'text-[#EA4335]' : 'text-white'}>PROPERTY</span>
                                    <span className={isSolid ? 'text-[#34A853]' : 'text-white'}>VLOG</span>
                                </h1>
                                <p className={`text-[9px] font-bold tracking-[0.2em] transition-colors ${isSolid ? 'text-slate-500' : 'text-slate-400 lg:text-blue-100'}`}>
                        ...
                        <div className="hidden lg:flex items-center gap-1 bg-white/5 backdrop-blur-sm px-2 py-1 rounded-full border border-white/10 shrink-0">
                            {NAVIGATION_CONFIG.map((item) => (
                                ...
                                >
                                    {item.children ? (
                                        <button
                                            className={`px-3 lg:px-4 py-2 rounded-full text-sm font-bold transition-all whitespace-nowrap flex items-center gap-1 ${isSolid ? 'text-slate-700 hover:bg-slate-100' : 'text-white hover:bg-white/10'}`}
                                        >
                                            {item.name}
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                            <a href={getWhatsAppUrl()} target="_blank" rel="noopener noreferrer" className={`hidden lg:flex items-center gap-2 px-3 lg:px-4 py-2 rounded-full font-bold text-sm transition-all ${isSolid ? 'bg-green-50 text-green-700 hover:bg-green-100 shadow-sm' : 'bg-green-500/20 text-white hover:bg-green-500/30 backdrop-blur-sm border border-green-400/30'
                                }`}>
                            ...
                            <a href="tel:+917744009295" className={`hidden lg:flex items-center gap-2 px-3 lg:px-4 py-2 rounded-full font-bold text-sm transition-all ${isSolid ? 'bg-slate-100 text-slate-900 hover:bg-slate-200' : 'bg-white/10 text-white hover:bg-white/20 backdrop-blur-sm border border-white/10'
                                }`}>
                                <Phone className="w-4 h-4" />
                                <span className="hidden xl:inline">+91 7744009295</span>
                            </a>

                            <button className={`p-2 rounded-full lg:hidden transition-colors ${isSolid ? 'text-slate-900' : 'text-white'}`} onClick={() => setMobileMenuOpen(true)}>
                        ...
                        >
                            ...
                            <div className="p-5 flex flex-col gap-2">
                                ...
                                <div className="mt-8 flex flex-col gap-3">
                                    <a href={getWhatsAppUrl()} className="w-full bg-[#25D366] text-white py-4 rounded-xl font-bold text-lg shadow-lg flex items-center justify-center gap-2">
